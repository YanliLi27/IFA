...
def namefinder(name_str:str, select_category:Union[None, str, int, list]=None, cam_method:str='gradcam', 
               ...
               feature_selection:bool=False, **kwarg) -> Tuple[str, str, str]:
    ...
    cam_dir = f'./output/{name_str}/cam/{cam_method}/scale{str(rescale)}_rm{str(remove_minus_flag)}_feature{str(feature_selection)}{kwarg}/cate{str(select_category)}'
    if not os.path.exists(cam_dir):
        os.makedirs(cam_dir)
    record_dir = f'./output/{name_str}/metric/{cam_method}/scale{str(rescale)}_rm{str(remove_minus_flag)}_feature{str(feature_selection)}{kwarg}/cate{str(select_category)}'
