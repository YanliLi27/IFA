...
import numpy as np
from predefined.raclip_components.generators.dataset.utils.resample import resampler
from sklearn.metrics import accuracy_score
from predefined.raclip_components.generators.scanner.id_scanner import img_id_scanner, ramris_id_filter
from predefined.raclip_components.generators.scanner.esmira_scanner import esmira_scanner
from predefined.raclip_components.generators.dataset.clipdataset import CLIPDataset
from predefined.raclip_components.generators.dataset.esmiradataset import ESMIRADataset
from predefined.raclip_components.generators.utils.distri_calculator import distri_calculators, corr_calculator, sca_calculator
from predefined.raclip_components.generators.utils.icc_calculator import icc_calculator
from predefined.raclip_components.generators.utils.record import corr_save
...
class ESMIRA_generator:
    def __init__(self, data_root:str, ramris_root:Union[str, None]=None,
                 target_category:list=['EAC', 'ATL'], target_site:list=['Wrist'], target_dirc:list=['TRA', 'COR'],
                 target_reader:list=['Reader1', 'Reader2'], target_biomarker:Union[list, None]=None, task_mode:str='class',
                 working_dir:str='.', print_flag:bool=False, maxfold:int=5, score_sum:bool=False) ->None:
        ...
        self.default_id_path = f'{self.working_dir}/generators/scanner/logs/{data_root[-4:]}_id_list.pkl'
        self.default_df_path = f'{self.working_dir}/generators/scanner/logs/{data_root[-4:]}_utli_df.pkl'
        ...
        if os.path.isfile(self.default_df_path):
            ...
            print(f'found saved dataframes in {self.default_df_path}')
        else:
        ...
        if task_mode in ['class', 'multi']:
            ...
            if os.path.isfile(target_img_save) and os.path.isfile(target_ramris_save) \
                ...
                print(f'--------found saved split in: {target_img_save} --------')
                ...
                target_id_split = self._split_generator(target_id_list, maxfold=maxfold)  # from [ids] to [5*[ids]]
                # atlas split generation
                atlas_id_list = atlas_df['Abs_ID']  # select id
                atlas_id_split = self._split_generator(atlas_id_list,  maxfold=maxfold)  # from [ids] to [5*[ids]]
                ...
                self.target_ramris_split, _ = self._ramris_loader(target_df, target_id_split_balanced, target_site,
                                                                target_biomarker, target_reader, wmae_flag=True)
            ...
            target_img_save = self._img_split_saver(target_category, target_site, target_dirc, True)
            target_ramris_save = self._ramris_split_saver(target_category, target_site, target_reader, target_biomarker, True, keyword='_sc')
            target_mae_save = self._ramris_split_saver(target_category, target_site, target_reader, target_biomarker, True, keyword='_mse')
            if os.path.isfile(target_img_save) and os.path.isfile(target_ramris_save):
                with open(target_img_save, "rb") as tf:
                    self.target_img_split = pickle.load(tf)
                with open(target_ramris_save, "rb") as tf:
                    self.target_ramris_split = pickle.load(tf)
                with open(target_mae_save, "rb") as tf:
                    self.target_mae = pickle.load(tf)
                print(f'--------found saved split in: {target_img_save}--------')
            else:
                # column filtering - site/dirc/reader
                ...
                self.target_ramris_split, self.target_mae = self._ramris_loader(target_df, target_id_split, target_site, target_biomarker, target_reader, wmae_flag=True)
                with open(target_img_save, "wb") as tf:
                    pickle.dump(self.target_img_split, tf)
                with open(target_ramris_save, "wb") as tf:
                    pickle.dump(self.target_ramris_split, tf)
                with open(target_mae_save, "wb") as tf:
                    pickle.dump(self.target_mae, tf)
            if self.print_flag:
                self._reader_corr_printer(self.target_ramris_split, self.target_mae)
    ...
    def _ramris_filter(self, path_list:list, num_reader:int=2, merge_mode:str='avg', nan_rep:int=0, wmae_flag:bool=False):
            ...
            for i in range(num_sites):
                ...
                for j, item in enumerate(ramris):
                    ...
                    if item < 0:
                        ramris[j] = -item
                paths.append(ramris)
            return paths if wmae_flag==False else paths, None # list [site1_array, site2_array]
        elif num_reader == 2:
            paths = []
                ...
                if wmae_flag == True:
                    ...
                    for i in range(num_sites):
                        ...
                        for j in range(len(reader1_score)):
                            if reader1_score[j] >= 10:
                                reader1_score[j] = nan_rep
                            if reader2_score[j] >= 10:
                                reader2_score[j] = nan_rep
                        avg = (reader1_score + reader2_score)/2  # (np.array + np.array)/2 -> np.array 
                        mae_indiv = avg - reader2_score
                        paths.append(avg)
                        wmae.append(mae_indiv)
                    return paths, wmae  # list [site1_array, site2_array], list[mae_value[split, split, ...], mae_value, mae_value]
                else:
                    for i in range(num_sites):
    ...
    def _ramris_loader(self, df:pd.DataFrame, id_split:list, target_site:list, target_bio:list, target_reader:list, wmae_flag:bool=False) ->Tuple[list, list]:
        # id_split [5 * split[ids]]  -- return [5 * split[id[path1:cs, path2:cs, path3:cs, ...]
        remain_columns = ['Abs_ID']
        for site in target_site:
        ...
        for split in id_split:
            ...
            for id in split:
                ...
                wmaes =  np.zeros((1, 0), dtype=np.float32)
                for i in range(len(paths)//2):
                    score, wmae = self._ramris_filter(paths[2*i:2*i+2], num_reader=len(target_reader), wmae_flag=wmae_flag)  # list [site1'array[]', site2'array[]']
                    scores = np.append(scores, score, axis=1)
                    wmaes = np.append(wmaes, wmae, axis=1)
                return_split.append(scores)  # list [id[site1_array, site2_array], id[[site1_array, site2_array]], ...]
                return_wmae.append(wmaes)  # list [id[site1_mae, site2_mae], id[site1_mae, site2_mae], ...]
                # weight_split.append(weight_evi)
            return_5_split.append(return_split)
            return_5_wmae.append(return_wmae)
            # weight_5_split.append(weight_split)
        return return_5_split, return_5_wmae  # return [5 * split[id[site1_array, site2_array], id[[site1_array, site2_array]] ...]
            # return2 [5 * split[id[site1_mae, site2_mae], id[[site1_mae, site2_mae]] ...]
    ...
    def _img_split_saver(self, target_category:list, target_site:list, target_dirc:list, target_flag:bool=True)->str:
        ...
        if target_flag:
            ...
            target_name = 0
        output_name = "{}/generators/scanner/recover/{}_{}_{}_{}_{}.pkl".format(self.working_dir, self.data_root[-4:], cate_name, site_name, reader_name, target_name)
        return output_name
    ...
    def _ramris_split_saver(self, target_category:list, target_site:list, target_reader:list,
        ...
        if target_biomarker:
            ...
            output_name = "{}/generators/scanner/recover/{}_{}_{}_{}_{}_{}_{}.pkl".format(self.working_dir, self.data_root[-4:], cate_name, site_name, t_b, reader_name, target_name, keyword)
        else:
            output_name = "{}/generators/scanner/recover/{}_{}_{}_{}_{}_{}.pkl".format(self.working_dir, self.data_root[-4:], cate_name, site_name, reader_name, target_name, keyword)
        return output_name
    ...
    def _mae_printer(self, train_mae_list:list, val_mae_list:list, len_ramris_site:int=43) ->Tuple[np.float32, np.float32, np.array, np.array]:
        # input: train list [id[site1_mae, site2_mae], id[site1_mae, site2_mae], id[mae], ...], val list [id[mae], id[mae], id[mae], ...]
        # -> output: float, float
        array_val = np.asarray(val_mae_list)  # [num_id, num_site, num_ramris_in_site (1)]
        array_all = np.asarray((train_mae_list + val_mae_list))
        print('the score shape:', array_val.shape)
        val_mae = np.mean(array_val).astype(np.float32)
        overall_mae = np.mean(array_all).astype(np.float32)
        ...
        for i in range(len_ramris_site):
            split_val_mae.append(np.mean(ndarray_val[:,i]))
            split_all_mae.append(np.mean(ndarray_all[:,i]))
        return overall_mae, val_mae, split_all_mae, split_val_mae
    

    def _corr_printer(self, score_array, mae_array, fig_path:str=f'./generators/background/dist.jpg', num_scores_per_site:int=43) -> Tuple[str, str, np.float32, np.float32]:
        ...
        # correlation
        try:
            corr_mae_score, p_value = corr_calculator(score_array, mae_array, num_scores_per_site, division=None)
        except:
            raise ValueError('correlation calculation failed')
        return distri_path, corr_mae_score, p_value


    def _reader_corr_printer(self, ramris_list:list, mae_list:list) ->None:
        # ramris_list [5 * split[id[site1_array, site2_array], id[[site1_array, site2_array]] ...]  (5, ids, num_scores)
        # mae_list [5 * split[id[site1_mae, site2_mae], id[[site1_mae, site2_mae]] ...] (5, ids, num_scores)
        extend_ramris = []
        extend_mae = []
        if len(ramris_list) >1:
            for item in ramris_list:
                extend_ramris.extend(item)  # [ids, num_scores]
            for mae in mae_list:
                extend_mae.extend(mae)  # [ids, num_scores]
        else:
            extend_ramris = ramris_list
            extend_mae = mae_list
        array_ramris = np.asarray(extend_ramris)
        array_mae = np.asarray(extend_mae)
        ...
        try:
            acc = accuracy_score(array_reader1.flatten(), array_reader2.flatten())
        except:
            acc = accuracy_score(np.rint(array_reader1.flatten()), np.rint(array_reader2.flatten()))
        icc = icc_calculator(np.sum(array_reader1, axis=-1), np.sum(array_reader2, axis=-1)) if self.score_sum \
            else icc_calculator(array_reader1, array_reader2)
        array_reader1 = np.sum(array_reader1, axis=-1)
        array_reader2 = np.sum(array_reader2, axis=-1)
        # get the correlation
        corr_mae_score, p_value = corr_calculator(array_reader1, array_reader2, array_ramris.shape[1], division=None)
        print('dataset readers correlation:', corr_mae_score)
        print('correlation p value:', p_value)
        print('reader accuracy:', acc)
        print('reader icc: ', icc)
        name_str = f'{self.target_site[0]}_{self.target_biomarker[0]}' if len(self.target_biomarker)==1 else f'{self.target_site[0]}_multiple'
        sca_calculator(Garray=array_reader1, Parray=array_reader2, num_scores_per_site=array_ramris.shape[1],
                        division=False, save_path=f'{self.working_dir}/generators/background/{name_str}_scatter_gt_pr.jpg')
        corr_save(icc, corr_mae_score, mode='icc', save_path=f'{self.working_dir}/generators/background/{name_str}_record.txt')


    def returner(self, task_mode:str='clip', phase:str='train', fold_order:int=0, material:Union[str, list]='img',
                 monai:bool=False, full_img:bool=False, dimension:int=2, 
                 data_balance:bool=True, path_flag:bool=False) ->Tuple[Union[None, Dataset], Dataset]:
        if dimension==2:
            if monai:
                ...
                transform = [transforms.Compose([transforms.RandomHorizontalFlip(0.5),
                            ...
                            transforms.Compose([
                                            transforms.RandomAffine(degrees=0, translate=(0.01, 0.01), scale=(1, 1), shear=None, fill=0),
                                            ])]
        else:
            from monai import transforms
            transform = [transforms.Compose([
        ...
        if task_mode == 'clip':
            return self._clip_returner(phase, fold_order, transform, full_img, dimension, data_balance, path_flag)
        elif task_mode == 'class':
            return self._class_returner(phase, fold_order, material, transform, dimension, path_flag)
        elif task_mode == 'multi':
            return self._multi_returner(phase, fold_order, material, full_img, transform, dimension, path_flag)


    def _clip_returner(self, phase:str='train', fold_order:int=0, transform=[None, None],
                       full_img:bool=False, dimension:int=2, data_balance:bool=True, 
                       path_flag:bool=False) ->Tuple[Union[None, Dataset], Dataset]:
        if phase=='train':
            ...
            train_mae_list, val_mae_list = self._split_definer(self.target_mae, fold_order)
            
            # data balance
            if data_balance:
                train_img_list, train_ramris_list, train_mae_list =\
                    resampler(train_img_list, train_ramris_list, train_mae_list)
            # val_img_list, val_ramris_list, val_mae_list =\
            #       resampler(val_img_list, val_ramris_list, val_mae_list)

            # statistic
            if self.print_flag:
                overall_mae, val_mae, split_all_mae, split_val_mae = self._mae_printer(train_mae_list, val_mae_list, self.len_ramris_site)
                print(f'overall mae: {overall_mae}, val mae: {val_mae}')
                # print(f'overall split mae: {split_all_mae}')
                # print(f'val split mae: {split_val_mae}')
                distri_all_path, corr_mae_score, p_value = self._corr_printer(np.asarray((train_ramris_list)), 
                                                                          np.asarray((train_mae_list)),
                ...
                distri_val_path, val_corr_mae_score, val_p_value = self._corr_printer(np.asarray(val_ramris_list), 
                                                                                  np.asarray(val_mae_list),
                ...
                print(f'corr of mae and rmaris in all is: {corr_mae_score}, p value is: {p_value}')
                print(f'for val the mae corr is {val_corr_mae_score}, val p value: {val_p_value}')

            # dataset
            train_dataset = CLIPDataset(self.data_root, train_img_list, train_ramris_list, transform[0], full_img, 
                                        dimension=dimension, score_sum=self.score_sum, path_flag=path_flag)
    ...
    def _class_returner(self, phase:str='train', fold_order:int=0, material:Union[str, list]='img',
                        transform=[None, None], dimension:int=2, path_flag:bool=False) ->Tuple[Union[None, Dataset], Dataset]:
        ...
        if phase=='train':
            ...
            train_dataset = ESMIRADataset(self.data_root, train_img_list, train_ramris_list, train_label,
                                           material, transform[0], dimension=dimension, path_flag=path_flag)
            val_dataset = ESMIRADataset(self.data_root, val_img_list, val_ramris_list, val_label,
                                         material, transform[1], dimension=dimension, path_flag=path_flag)
        else:
    ...
    def _multi_returner(self, phase:str='train', fold_order:int=0, material:Union[str, list]='img', full_img:bool=False,
                        transform=[None, None], dimension:int=2, path_flag:bool=False) ->Tuple[Union[None, Dataset], Dataset]:
        ...
        if phase=='train':
            ...
            val_ramris_list, _ = self._ta_mixer(target_val_ramris_list, atlas_val_ramris_list)
            train_dataset = ESMIRADataset(self.data_root, train_img_list, train_ramris_list, train_label,
                                           material, full_img, transform[0], dimension=dimension, path_flag=path_flag)
            val_dataset = ESMIRADataset(self.data_root, val_img_list, val_ramris_list, val_label,
                                         material, full_img, transform[1], dimension=dimension, path_flag=path_flag)
        else:
            target_val_img_list = self._val_split_definer(self.target_img_split)
            atlas_val_img_list = self._val_split_definer(self.atlas_img_split)
            val_img_list, val_label = self._ta_mixer(target_val_img_list, atlas_val_img_list)
