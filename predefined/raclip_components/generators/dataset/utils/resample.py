...
def resampler(img_list:list, ramris_list:list, mae_list:list, groups:int=10) -> Tuple[list, list, Union[None, list]]:
    ...
    ratio_list = np.minimum(ratio_list, 5)
    # repete
    extra_img_list = []
    extra_ramris_list = []
    extra_mae_list = []
    ...
    assert len(mae_list) == len(img_list)
    for index in range(len(img_list)):
        ...
        for _ in range(repeat):
            ...
            extra_mae_list.append(mae_list[index])

    print('img and ramris balanced')
    return extra_img_list, extra_ramris_list, extra_mae_list
