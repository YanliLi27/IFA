...
def _scatter_plot(Garray:np.array, Parray:np.array, mode:str='save', save_path:str='./models/figs/scatter_gt_pr.jpg'):
    ...
    max_value = np.minimum(np.max(Garray), np.max(Parray))
    x_s=np.arange(0,max_value)
    plt.plot(x_s,linear_model_fn(x_s),color="red")
    # set the reference curve
    plt.plot([0, max_value], [0, max_value],color="green")
    # scatter plot
    plt.scatter(Garray, Parray, s=20, c=None, marker=None, cmap=None, norm=None, linewidths=None)
    plt.savefig(save_path)
    plt.clf()
    plt.close()
    # fig with not curve
    plt.clf()
    plt.scatter(Garray, Parray, s=20, c=None, marker=None, cmap=None, norm=None, linewidths=None)
    plt.savefig(save_path.replace('.jpg', '_no.jpg'))
    plt.clf()
    plt.close()
    # save the G and P
    Savearray = np.vstack((Garray, Parray))
    np.save(save_path.replace('.jpg', '.npy'),Savearray)
    print('Scatter plot and npy saved')
